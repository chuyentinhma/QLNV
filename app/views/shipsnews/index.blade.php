...
<div class="wrap">
    <div class='head'>
        ...
        <div class='page-tools'>
            <a class='btn btn-primary' href='{{route('shipsnews.create')}}'>
                <i class="i-plus-2"></i>
                Giao tin
            </a>
                ...
                <div class='content np table-sorting'>
                    <table cellpadding='0' cellspacing='0' class='sort' width='100%'>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td >{{ ++$index }}</td>
                                <td >{{\Carbon\Carbon::parse($new->date_submit)->format('d/m/Y')}}</td>
                                <td >
                                    {{$new->customer->order->number_cv . '/' .$new->customer->order->unit->symbol}}
                                </td>
                                <td>{{$new->customer->order->number_cv_pa71}}</td>
                                <td>{{$new->customer->order->customer_name}}</td>
                                <td>{{$new->customer->phone_number}}</td>
                                <td>{{$new->customer->order->category->symbol}}</td>
                                <td>{{$new->customer->order->kind->symbol}}</td>
                                <td>
                                    @foreach($new->customer->order->purposes as $purpose)
                                    {{$purpose->content}}<br>
                                    @endforeach
                                </td>
                                <td>{{$new->news_number}}</td>
                                <td>{{$new->page_number}}</td>
                                <td>{{$new->user->username}}</td>
                                <td>{{$new->receive_name}}</td>
                                <td class="text-center" >
                                    <a class="btn btn-warning btn-mini" href="{{route('shipsnews.edit',$new->id)}}" title="Sửa">
                                        <i class="icon-edit"></i>
                                    </a>
                                    <a class="btn btn-danger btn-mini" data-confirm="Bạn có chắc chắn muốn xóa bản ghi này" btn-confirm="confirm" data-url="{{route('shipsnews.delete',$new->id)}}" title="Xóa">
